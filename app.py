    ...
    with app.app_context():
        ...
        try:
            ...
            case_data = {
                ...
                'date_today': datetime.now().strftime('"%d" %B %Y г.'),
                'loan_amount': '100000',
                'due_date': '15.04.2024',
...
def upload_documents():
    ...
    if balance['credits_remaining'] < 1:
        return error_response(
            'Недостаточно кредитов для создания дела. Пополните баланс.', 
    ...
    # Deduct credit
    success, remaining_credits = deduct_credit(user.id, count=1)
    if not success:
        return error_response(
            'Недостаточно кредитов для создания дела. Пополните баланс.', 
            'INSUFFICIENT_CREDITS', 
    ...
    # Check for balance notifications after credit deduction
    check_balance_notifications(user.id)
    
    logger.info(f"Documents uploaded for case {case.id}: {len(uploaded_files)} files. Credits remaining: {remaining_credits}")
...
@require_auth
def get_balance():
    """Get user's current credit balance and tariff info."""
    log_request_info()
    user = request.current_user
